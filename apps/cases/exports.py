        ...
        if evidence:
            ...
            if evidence:
                ...
                for item in evidence:
                    _append_line(pdf, f"- {item.label}: {_compact_reference(item.storage_path or 'on file')}")

        pdf.ln(2)
        footer_text = _safe_text(
            f"Generated by LexGuard on {timezone.localtime(timezone.now()).strftime('%Y-%m-%d %H:%M')}. Deterministic export; no automated inference applied."
        )
        footer_height = 5
        if pdf.get_y() + footer_height <= pdf.h - getattr(pdf, 'page_break_trigger', getattr(pdf, 'page_break_margin', 15)):
            pdf.set_font("Helvetica", "I", 8)
            pdf.set_text_color(110, 116, 125)
            pdf.cell(0, footer_height, footer_text, ln=True)
